...
function setup(role: "admin" | "student" | null = "admin", available = true) {
  const repository = { list: vi.fn().mockResolvedValue({ items: [], total: 0, cohortYears: [2023] }), detail: vi.fn().mockResolvedValue({ id, profileStatus: "missing" }) };
  const authorization = { isAdmin: vi.fn().mockResolvedValue(true) };
  const app = express();
...
describe("student profile read API", () => {
  ...
  it("rejects stale Admin sessions using current database authorization", async () => {
    const { app, authorization, repository } = setup(); authorization.isAdmin.mockResolvedValue(false);
  ...
  it("returns a no-store paginated response", async () => {
    ...
    expect(result.body).toEqual({ items: [], total: 0, cohortYears: [2023], page: 1, pageSize: 10 });
