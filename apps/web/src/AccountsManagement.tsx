...
import { CreateAccount, Icon, readResponse, type Account, type AccountPage } from "./admin-account-shared";
...
const roleName = (role: string) => role === "admin" ? "Quản trị viên" : "Sinh viên";

function History({ userId }: { userId?: string }) {
  const [draft, setDraft] = useState({ q: "", outcome: "", portal: "", from: "", to: "" });
  ...
  return <section aria-label="Lịch sử đăng nhập"><p className="am-table-note"></p>
    <form className="am-filters" onSubmit={(event) => { event.preventDefault(); setPage(1); setFilters({ ...draft, q: draft.q.trim() }); state.retry(); }}>
      <label>Từ khóa<input placeholder="Tên, email, mã kết quả…" maxLength={120} value={draft.q} onChange={(e) => setDraft({ ...draft, q: e.target.value })} /></label>
      ...
      <button className="am-outline">Áp dụng bộ lọc</button><button className="am-quiet" type="button" onClick={() => { const empty = { q: "", outcome: "", portal: "", from: "", to: "" }; setDraft(empty); setFilters(empty); setPage(1); state.retry(); }}>Xóa bộ lọc</button>
    </form>
...
function ChangeAccount({ account, mode, onClose, onSaved }: { account: Account; mode: "role" | "lock"; onClose: () => void; onSaved: () => void }) {
  ...
  return <Modal title={title} onClose={onClose} busy={busy}><form className="am-change-form" onSubmit={(event) => {
    ...
    {mode === "role" && <label className="am-field">Vai trò mới<select value={role} disabled={busy} onChange={(e) => setRole(e.target.value as Account["role"])}><option value="student">Sinh viên</option><option value="admin">Quản trị viên</option></select></label>}
...
export default function AccountsManagement({ actorId }: { actorId: string }) {
  const [draft, setDraft] = useState({ q: "", role: "", active: "" });
  const [filters, setFilters] = useState(draft);
  const [page, setPage] = useState(1);
  const [revision, setRevision] = useState(0);
  const [tab, setTab] = useState<"accounts" | "history">("accounts");
  const [modal, setModal] = useState<{ mode: "detail" | "role" | "lock" | "history"; account: Account } | null>(null);
  ...
  return <><section className="am-card"><div className="am-card-heading"><div><h2>Tài khoản người dùng {state.data && <span className="am-count">{state.data.total}</span>}</h2><p>Tài khoản, phân quyền và lịch sử đăng nhập trong cùng một nơi.</p></div><button className="am-primary" onClick={() => setCreating(true)}><Icon name="add" />Tạo tài khoản quản trị</button></div>
    <div className="am-view-switch" role="group" aria-label="Chế độ xem"><button aria-pressed={tab === "accounts"} onClick={() => setTab("accounts")}>Danh sách tài khoản</button><button aria-pressed={tab === "history"} onClick={() => setTab("history")}>Lịch sử đăng nhập</button></div>
    {notice && <p className="am-notice" role="status">{notice}</p>}
    {tab === "history" ? <History /> : <><form className="am-filters" onSubmit={(event) => { event.preventDefault(); setPage(1); setFilters({ ...draft, q: draft.q.trim() }); refreshed(); }}>
      <label>Tìm kiếm<input placeholder="Họ tên hoặc email…" maxLength={120} value={draft.q} onChange={(e) => setDraft({ ...draft, q: e.target.value })} /></label>
      <label>Vai trò<select value={draft.role} onChange={(e) => setDraft({ ...draft, role: e.target.value })}><option value="">Tất cả vai trò</option><option value="admin">Quản trị viên</option><option value="student">Sinh viên</option></select></label>
      ...
      <button className="am-outline">Tìm kiếm / Lọc</button><button className="am-quiet" type="button" onClick={() => { const empty = { q: "", role: "", active: "" }; setDraft(empty); setFilters(empty); setPage(1); refreshed(); }}>Làm mới</button>
    </form><Status {...state} />{state.data && <><div className="am-table-scroll" tabIndex={0} role="region" aria-label="Bảng tài khoản người dùng"><table className="am-table"><thead><tr><th>STT</th><th>Người dùng</th><th>Vai trò</th><th>Trạng thái</th><th>Đăng nhập gần nhất</th><th>Thao tác</th></tr></thead><tbody>{state.data.items.map((account, index) => <tr key={account.id}><td>{(page - 1) * 10 + index + 1}</td><td><div className="am-person"><span className="am-avatar" aria-hidden="true">{account.name.slice(0, 1)}</span><div><button className="am-name-link" onClick={() => setModal({ mode: "detail", account })}>{account.name}</button><small>{account.email ?? account.username}</small></div></div></td><td><span className={`am-badge ${account.role === "admin" ? "am-badge-admin" : ""}`}>{roleName(account.role)}</span></td><td><span className={`am-state ${account.isActive ? "" : "am-state-locked"}`}>{account.isActive ? "Đang hoạt động" : "Đã khóa"}</span></td><td>{date(account.lastLoginAt)}</td><td><div className="am-row-actions"><button className="am-outline" onClick={() => setModal({ mode: "detail", account })}>Chi tiết</button><button className="am-outline" disabled={account.id === actorId} title={account.id === actorId ? "Không thể tự đổi quyền" : undefined} onClick={() => setModal({ mode: "role", account })}>Phân quyền</button><button className="am-outline" disabled={account.id === actorId} title={account.id === actorId ? "Không thể tự khóa tài khoản" : undefined} onClick={() => setModal({ mode: "lock", account })}>{account.isActive ? "Khóa" : "Mở khóa"}</button><button className="am-outline" onClick={() => setModal({ mode: "history", account })}>Lịch sử</button></div></td></tr>)}</tbody></table></div>{!state.data.items.length && <p className="am-empty">Không tìm thấy tài khoản phù hợp.</p>}<Pagination total={state.data.total} page={page} setPage={setPage} /></>}
