...
type Profile = { name: string; studentCode: string | null; cohortCode: string | null; className: string | null; cohortYear: number | null; currentSemester: number | null; email: string | null; careerGoal: string | null; profileStatus: string };
  ...
  return <div className="sw-shell">
    ...
    <div className="sw-workspace">
      ...
      <main className="sw-main" id="student-main" tabIndex={-1}>
        ...
        </> : page === "profile" ? <div className="sw-content-grid"><div>{profileCard}<section className="sw-panel"><div className="sw-section-heading"><h2>Bảng điểm của tôi</h2><span className="sw-tag">Đang phát triển</span></div><div className="sw-empty"><Icon name="book" /><h3>Chưa có bảng điểm để hiển thị</h3><p>Chức năng import và cập nhật bảng điểm sẽ được bổ sung tại đây.</p></div></section></div><aside><section className="sw-panel"><h2>Mục tiêu nghề nghiệp</h2><p className="sw-muted">{profile?.careerGoal || "Bạn chưa cập nhật mục tiêu nghề nghiệp."}</p><button className="sw-text-link" onClick={() => navigate("career")}>Khám phá nghề nghiệp <Icon name="arrow" /></button></section></aside></div> : <section className="sw-panel sw-feature"><span className="sw-card-icon"><Icon name={selected.icon} /></span><span className="sw-tag">Đang phát triển</span><h2>{featureCopy[page]?.title}</h2><p>{featureCopy[page]?.text}</p><div className="sw-feature-steps">{featureCopy[page]?.steps.map((step, i) => <div key={step}><span>0{i + 1}</span><h3>{step}</h3></div>)}</div><p className="sw-muted">Chức năng này chưa sẵn sàng sử dụng. Bạn có thể xem thông tin học tập hiện có trong hồ sơ.</p><button className="sw-primary" onClick={() => navigate("profile")}>Xem hồ sơ học tập <Icon name="arrow" /></button></section>}
