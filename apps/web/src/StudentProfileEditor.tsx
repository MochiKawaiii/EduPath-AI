...
export default function StudentProfileEditor({ profile, onSaved }: { profile: EditableStudentProfile; onSaved: () => void }) {
  ...
  const [draft, setDraft] = useState({ className: "", interests: "", careerGoal: "" });
  const [busy, setBusy] = useState(false);
  const [message, setMessage] = useState("");
  const [error, setError] = useState("");
  useEffect(() => { if (!editing) setDraft({ className: profile.className ?? "", interests: profile.interests ?? "", careerGoal: profile.careerGoal ?? "" }); }, [profile, editing]);
  const save = async (event: FormEvent) => {
    event.preventDefault(); setBusy(true); setError(""); setMessage("");
    try {
      const response = await fetch("/api/student/profile", { method: "PATCH", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify(draft) });
