...
type Student = { id: string; name: string; email: string | null; isActive: boolean; studentCode: string | null; cohortYear: number | null; currentSemester: number | null; profileStatus: ProfileStatus };
...
function StudentDetails({ id }: { id: string }) {
  ...
  return <><Status {...state} />{student && <div className="sp-detail">
    ...
    <dl className="am-detail-grid">{Object.entries({ "Họ và tên": student.name, "Mã sinh viên": student.studentCode, "Email": student.email, "Tên đăng nhập": student.username, "Năm nhập học": student.cohortYear, "Học kỳ hiện tại": student.currentSemester, "Trạng thái tài khoản": student.isActive ? "Đang hoạt động" : "Đã khóa", "Trạng thái hồ sơ": statusLabels[student.profileStatus] }).map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value ?? "Chưa cập nhật"}</dd></div>)}</dl>
...
export default function StudentProfiles() {
  ...
  return <><section className="am-card" aria-labelledby="student-list-title">
    ...
    <form className="am-filters sp-filters" onSubmit={(event) => { event.preventDefault(); setFilters({ ...draft, q: draft.q.trim() }); setPage(1); state.retry(); }}>
      <label className="sp-search" htmlFor="student-search">Từ khóa<input id="student-search" maxLength={120} value={draft.q} onChange={(e) => setDraft({ ...draft, q: e.target.value })} placeholder="Mã sinh viên, họ tên, email, mục tiêu…" /></label>
      ...
      <label htmlFor="student-semester">Học kỳ hiện tại<select id="student-semester" value={draft.semester} onChange={(e) => setDraft({ ...draft, semester: e.target.value })}><option value="">Tất cả học kỳ</option>{Array.from({ length: 20 }, (_, i) => <option key={i + 1} value={i + 1}>Học kỳ {i + 1}</option>)}</select></label>
    ...
    <Status {...state} />{state.data && <><div className="am-table-scroll" tabIndex={0} role="region" aria-label="Bảng hồ sơ sinh viên, cuộn ngang trên màn hình nhỏ"><table className="am-table sp-table"><thead><tr><th scope="col">STT</th><th scope="col">Sinh viên</th><th scope="col">Mã sinh viên</th><th scope="col">Năm nhập học</th><th scope="col">Học kỳ</th><th scope="col">Hồ sơ</th><th scope="col">Tài khoản</th><th scope="col">Thao tác</th></tr></thead><tbody>{state.data.items.map((student, index) => <tr key={student.id}><td>{(page - 1) * 10 + index + 1}</td><td><div className="am-person"><span className="am-avatar" aria-hidden="true">{student.name.slice(0, 1)}</span><div><button className="am-name-link" onClick={() => setSelected(student)}>{student.name}</button><small>{student.email ?? "Chưa có email"}</small></div></div></td><td>{student.studentCode ?? "Chưa cập nhật"}</td><td>{student.cohortYear ?? "—"}</td><td>{student.currentSemester ?? "—"}</td><td><ProfileBadge status={student.profileStatus} /></td><td><span className={`am-state ${student.isActive ? "" : "am-state-locked"}`}>{student.isActive ? "Đang hoạt động" : "Đã khóa"}</span></td><td><button className="am-outline" onClick={() => setSelected(student)}>Xem hồ sơ</button></td></tr>)}</tbody></table></div>
      {!state.data.items.length && <div className="am-empty"><h3>{filtered ? "Không tìm thấy hồ sơ phù hợp" : "Chưa có sinh viên trong danh sách"}</h3><p>{filtered ? "Thử từ khóa khác hoặc xóa bộ lọc." : "Danh sách sẽ hiển thị khi có tài khoản mang vai trò Sinh viên trong tổ chức của bạn."}</p></div>}
