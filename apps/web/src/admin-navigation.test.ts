...
describe("admin section navigation", () => {
  it("has account, student-profile, curriculum, graduation, and training-plan sections", () => {
    expect(adminNavigation.map(item => item.path)).toEqual(["/quantri/tai-khoan", "/quantri/sinh-vien", "/quantri/chuong-trinh", "/quantri/tieu-chuan-tot-nghiep", "/quantri/ke-hoach"]);
  });
